...
type LayerProps = Omit<AtlasProps, 'center' | 'spanM' | 'dim' | 'className' | 'children'> & {
  project: Projector;
  /** Draw the paper land + graticule behind everything (SVG-only map). */
  background?: boolean;
};

export const LENS_MIN_M = 200;
export const LENS_MAX_M = 4000;
...
export function AtlasLayer({
  ...
  const onLensPointer = useCallback(
    (e: { clientX: number; clientY: number }) => {
      if (!home || !onLensChange || !svgRef.current) return;
      ...
      onLensChange(Math.max(LENS_MIN_M, Math.min(LENS_MAX_M, Math.round(m / 50) * 50)));
    },
  ...
  const threadPts = useMemo(() => {
    ...
    thread.forEach((n, i) => {
      if (n.point) {
        ...
        const a = prev ? project(prev) : { x: width / 2, y: height / 2 };
    ...
    return pts;
  }, [thread, project, width, height]);

  return (
    <svg
      ref={svgRef}
      className={`papermap__svg${background ? '' : ' papermap__svg--overlay'}`}
      width={width}
      height={height}
      onClick={background ? () => onSelectPlace?.(null) : undefined}
    >
      {background ? (
        <>
          <defs>
            <pattern id="graticule" width={gridPx} height={gridPx} patternUnits="userSpaceOnUse" x={width / 2} y={height / 2}>
        ...
        <g className="lens">
          ...
          {onLensChange ? (
            <circle
              ...
              aria-valuenow={lensM}
              aria-valuemin={LENS_MIN_M}
              aria-valuemax={LENS_MAX_M}
              tabIndex={0}
              onKeyDown={(e) => {
                if (e.key === 'ArrowRight' || e.key === 'ArrowUp') onLensChange(Math.min(LENS_MAX_M, lensM + 100));
                if (e.key === 'ArrowLeft' || e.key === 'ArrowDown') onLensChange(Math.max(LENS_MIN_M, lensM - 100));
              }}
            />
          ) : null}
          <text x={homePx.x + lensM * project.scale * Math.SQRT1_2 + 12} y={homePx.y - lensM * project.scale * Math.SQRT1_2 + 4} className="lens__label">
