...
/** A place picked from search: a locality or a city centre. */
...
type Action =
  | { type: 'setHome'; home: HomePick | null }
  | { type: 'setAnchor'; anchor: HomePick | null }
...
const defaultImportance = Object.fromEntries(CATEGORY_KEYS.map((k) => [k, CATEGORIES[k].defaultImportance])) as Record<CategoryKey, Importance>;
...
function reducer(s: LocusState, a: Action): LocusState {
  switch (a.type) {
    ...
    case 'removePlace':
      return s.removedPlaceIds.includes(a.placeId) ? s : { ...s, removedPlaceIds: [...s.removedPlaceIds, a.placeId], match: null };
    case 'undoRemove':
      return { ...s, removedPlaceIds: s.removedPlaceIds.slice(0, -1), match: null };
    case 'toggleRoutineNode':
      return {
        ...s,
        droppedRoutineNodes: s.droppedRoutineNodes.includes(a.node) ? s.droppedRoutineNodes.filter((n) => n !== a.node) : [...s.droppedRoutineNodes, a.node],
      };
    case 'setLens':
...
export function LocusProvider({ children }: { children: ReactNode }) {
  ...
  const derived = useMemo<Derived>(() => {
    ...
    const all = state.baseline?.places ?? [];
    const places = all.filter((p) => !removed.has(p.placeId));
    const counts = { ...(state.baseline?.counts ?? (Object.fromEntries(CATEGORY_KEYS.map((k) => [k, 0])) as Record<CategoryKey, number>)) };
    for (const p of all) if (removed.has(p.placeId)) counts[p.category] = Math.max(0, counts[p.category] - 1);
    const categories = CATEGORY_KEYS.map((k) => ({ category: k, baselineCount: counts[k], importance: state.importance[k] }));
    const dropped = new Set(state.droppedRoutineNodes);
    const routine = deriveRoutine(categories, state.anchor != null).filter((n) => n === 'home' || !dropped.has(n));
